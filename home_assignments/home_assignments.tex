...
\item {[For Fun]} What is the expected number of pints of beer Joe will drink?
\end{enumerate}
...
If it shows 2 then I immediately get 2 roubles [plus the sum in the hat] and the game stops. 
...
Maybe useful for the midterm exam ;)

\begin{enumerate}
\item A monkey is randomly typing letters. 
She types $a$ with probability $0.01$, $b$ — with probability $0.02$ and $c$ — with probability $0.03$.
Other probabilities are unknown and not relevant. 
She stops if starting exactly from the beginning she types one of the words: $abc$, $bacab$ or $abba$. 
Let $A$ be the event that she stops. 

Consider the generating function $f(a, b, c) = abc + bacab + abba$.
What is the combinatorial or probabilistic meaning of the following quantities:
\begin{enumerate}
  \item $f(1, 1, 1)$;
  \item $f(0.01, 0.02, 0.03)$;
  \item $g'(1)$ if $g(t) = f(0.01t, 0.02t, 0.03t)$;
  \item $g'(1)$ if $g(t) = f(0.01, 0.03, 0.03t)$;
  \item $g'(1)/g(1)$ if $g(t) = f(0.01t, 0.02t, 0.03t)$;
  \item $g'(1)/g(1)$ if $g(t) = f(0.01, 0.03, 0.03t)$;
  \item $g'(0)$ if $g(t) = f(0.01\exp(t), 0.02\exp(t), 0.03\exp(t))$.
  \item $g'(0) / g(0)$ if $g(t) = f(0.01, 0.02, 0.03\exp(t))$.
\end{enumerate}

\item A monkey is randomly typing letters. 
She types $a$ with probability $0.01$, $b$ — with probability $0.02$ and $c$ — with probability $0.03$.
Other probabilities are unknown and not relevant. 
She stops if starting exactly from the beginning she types one of the words: $abc$, $bacab$, $abba$, \dots{ } 
Let $A$ be the event that she stops. 

You don't know the exact list of words but you can do any calculations with the generating function  function $f(a, b, c) = abc + bacab + abba + \dots$.

How can you extract the following information:
\begin{enumerate}
  \item the number of words;
  \item the probability that monkey stops;
  \item the expected payoff when she gets one dollar for every letter typed in the case of finite game;
  \item the expected payoff when  she gets one dollar for every letter $c$ typed in the case of finite game;  
  \item the conditional expected payoff when she gets one dollar for every letter typed if it is known that the game ended in a finite number of steps;
  \item the conditional expected payoff when she gets one dollar for every letter $c$ typed if it is known that the game ended in a finite number of steps;
  \item $\E(N_a N_b I(A))$ where $N_a$ is the number of $a$ letters, $N_b$ — the number of $b$ letters and $I(A)$ — the indicator of a finite game.
  \item $\E(N_a (N_a - 1) N_b (N_b - 1) (N_b - 2) I(A))$ where $N_a$ is the number of $a$ letters, $N_b$ — the number of $b$ letters and $I(A)$ — the indicator of a finite game.
  \item $\E(N_a^2 N_b^3 I(A))$ where $N_a$ is the number of $a$ letters, $N_b$ — the number of $b$ letters and $I(A)$ — the indicator of a finite game.
\end{enumerate}
Hint: in points (c) and (d) try to write the expression with and without exponent.
...
Enters in the quizzes during the week 5: 29 september — 4 october. 


\begin{enumerate}
\item The moment generating function of a random variable $X$ is $1/(1-2t)$.
  \begin{enumerate}
      ...
      \item Do you remember the sum of geometric progression? Find $\E(X^{2025})$.
  \end{enumerate}

\item The MGF of the random variable $W$ has a Taylor expansion that starts with $M(t) = 1 + 2t + 7t^2 + 20t^3 + \ldots$.

\begin{enumerate}
    \item Find $\E(W)$, $\Var(W)$, $\E(W^3)$.
    \item Find the starting terms of the Taylor expansion of the moment generating function for the sum $S = W + W^*$, where $W$ and $W^*$ are independent and identically distributed. 
\end{enumerate}



\item There are three problems in the home assignment. 
...
\item Let  $W = \sum_{i=1}^N X_i$,  where the $X_i$'s are independent and identically distributed Bernoulli random variables with probability of success $p$. 
The random variable $N$ has a binomial distribution with parameter $n$ and probability of success $p$, and is independent of the $X_i$'s.

  \begin{enumerate}
    \item Find the moment generating function for $X_i$.
    \item Find the moment generating function for $W$ given that $N$ is fixed. 
    \item Find the moment generating function for $W$.
    \item State the probability mass function of $W$. 
  \end{enumerate}
...
Enters in the quizzes during the week 6: 6–11 october. 


\begin{enumerate}
\item Consider the following Markov chain:

\begin{tikzpicture}[state/.style={circle, draw, minimum size=10mm}]
...
(2) edge[bend left] node[left] {0.7} (5)
(3) edge node {1} (6)
(5) edge[bend left] node {0.1} (4)
(4) edge[loop left] node {1} (4)
(6) edge[bend left] node {0.5} (5)
(5) edge[bend left] node {0.2} (6)
(5) edge node {0.7} (9)
(6) edge node {0.5} (7)
(7) edge[bend left] node {1} (8)
(8) edge[bend left] node {0.6} (7)
(8) edge[loop right] node {0.4} (8)
(9) edge node {1} (10)
(10) edge node {1} (11)
(11) edge[bend left] node {1} (9);

\end{tikzpicture}


\begin{enumerate}
    \item Cut the Markov chain into communicating classes and classify them as transient, positive recurrent or null-recurrent. 
...
Live in quizzes: 3–8 October. 

\begin{minipage}{0.9\textwidth}
        \includegraphics[scale=0.35]{figures/georgia_shuamta.jpg}  
\end{minipage}

\begin{enumerate}
\item  Let $X_n$ be a discrete time stochastic process that converges in probability to a random variable $X$ as $n\to \infty$. 
  \begin{enumerate}
    \item Does this condition imply that $X_n$ converges to $X$ in mean? Almost surely? In distribution? Support your answers with a proof or counterexample. 
    \item Give a definition for each type of convergence.
  \end{enumerate}  

\item The random variables $X_i$ are independent and exponentially distributed with rate $\lambda = 1$. 
...
\item The random variables  $X_1$, $X_2$, $X_3$, \ldots{} have the cumulative distribution functions
\[
F_{n}(x) =
\begin{cases}
1 - (1 - 1/n)^{nx}, \text{ if } x > 0, \\
0,  \text{ otherwise.}
\end{cases}
\]
Find the limit of the sequence $(X_n)_{n=1}^{\infty}$ in distribution.
...
Live in quizzes: 1 December. 

\begin{minipage}{0.9\textwidth}
        \includegraphics[scale=0.85]{figures/roman_numerals_2x.png}  
\end{minipage}


\begin{enumerate}
    \item Let $\Omega = \{a, b, c\}$ and the distribution of $Y$ is given by the table 
  ...
  The sigma-algebra $\cF$ is defined by $\cF = \sigma(A)$ where $A = \{Y \neq 4 \}$, and $\cH = \sigma(Y)$.

  \begin{enumerate}
    \item Find $\E(Y \mid \cF)$, $\Var(Y \mid \cF)$, $\P(Y = 3 \mid \cF)$.
    \item Find $\E(Y \mid \cH)$, $\Var(Y \mid \cH)$, $\P(Y = 3 \mid \cH)$.
  \end{enumerate}
...
\begin{enumerate}
\item  Explicitely list all the events in sigma-algebra $\sigma(X_1)$ and $\sigma(X_1 \cdot X_2)$.
\item  Pavel says that he knows only whether $X_1$ and $X_3$ are equal. 
How will you describe his knowledge with sigma-algebra?
\item  How many events are in the sigma-algebra $\sigma(X_1, X_1 + X_2, X_1 + X_2 + X_3)$?
\end{enumerate}


%    \item The random variables $X$ and $Y$ are independend with Poisson distribution with rate $\lambda = 1$.
%Let $S = X + Y$.
...
Live in quizzes: 15 December. 

\begin{minipage}{0.9\textwidth}
        \includegraphics[scale=0.75]{figures/xkcd_688_self_description.png}  
\end{minipage}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Find $\E(W_5 W_6 \mid W_5)$, $\Var(W_5 W_6 \mid W_5)$.
        \item Find $\E(W_7^6)$.
        \item Find $\E(W_3^2 W_5^2)$.
        \item What is the distribution of $R = 5W_2 + 6W_7$?
    \end{enumerate}
    Hint: Stein lemma, Isserlis theorem.
    \item Let $(W_t)$ be a standard Wiener process.  Consider the process $Q_t$:
    \[
    Q_t = \begin{cases}
        1, \text{ if } t < 2, \\
        W_1^2, \text{ if } t \geq 2.
    \end{cases}
    \]
    \begin{enumerate}
        \item Find the integral $I = \int_0^5 Q_s \, dW_s$ in terms of values of $(W_t)$.
        \item Find $\E(I)$ and $\Var(I)$.
    \end{enumerate}
\end{enumerate}
